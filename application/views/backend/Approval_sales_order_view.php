...
<!--main content start-->
    <section id="main-content">
        <section class="wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="glyphicon glyphicon-folder-open"></i> APPROVAL SALES ORDER</h3>
				</div>
			</div>
            <!-- page start-->
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('approval/ajax_list_so')?>",
            "type": "POST"
        },
...
function delete_data(nomor_po)
{
    if(confirm('Apakah kamu yakin akan menghapus data?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('purchase_order/delete_PO')?>/"+nomor_po,
            type: "POST",
            dataType: "JSON",
...
$(document).on('click', '#LihatDetailTransaksi', function(e){
		...
		$.ajax({
			url: "<?php echo site_url('approval/detail_so');?>",
			type: "POST",
			cache: false,
			data: { 
			nomor_so : $(this).text()
			...
			success: function(data){
				...
				jQuery.each(data['master'], function(obj, values) {
				   ...
				   //console.log(data['master'].nomor_so);
					...
					$('.modal-body').append('</tr>');
					$('.modal-body').append('<tr>');
					$('.modal-body').append('<td style="width:120px;">Nama Customer</td>');
					$('.modal-body').append('<td style="width:20px;">:</td>');
				...
				jQuery.each(data['detail'], function(obj, values) {
					...
					$('.modal-body').append('<td>Total SO<td>');
					$('.modal-body').append('<td colspan="3" style="text-align:left;">'+to_rupiah(total_so)+'<td>');
					$('.modal-body').append('</tr>');
					$('.modal-body').append('<tr>');
					$('.modal-body').append('<td colspan="4"><td>');
...
function approve_order(nomor_so)
{
	//alert(nomor_so);

	// ajax delete data to database
    $.ajax({
        url : "<?php echo site_url('approval/approve_so')?>/",
        type: "POST",
		data: { 
			nomor : nomor_so
			},
...
function cancel_order(nomor_so)
{
    if(confirm('Apakah kamu yakin akan membatalkan order?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('approval/cancel_so')?>",
            type: "POST",
			data: { 
			nomor : nomor_so
			},
