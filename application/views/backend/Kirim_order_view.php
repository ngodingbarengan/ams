...
<!--main content start-->
    <section id="main-content">
        <section class="wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="glyphicon glyphicon-folder-open"></i> KIRIM ORDER HISTORY</h3>
				</div>
			</div>
            <!-- page start-->
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('kirim_order/ajax_list')?>",
            "type": "POST"
        },
...
$(document).on('click', '#LihatDetailTransaksi', function(e){
		...
		$.ajax({
			url: "<?php echo site_url('kirim_order/detail_so');?>",
			type: "POST",
			cache: false,
			data: { 
			nomor_so : $(this).text()
			...
			success: function(data){
				...
				jQuery.each(data['master'], function(obj, values) {
				   ...
				   $('#btnPrint').attr('onclick', 'window.open("<?php echo site_url('kirim_order/get_invoice');?>/'+data['master'].id_order+'", "_blank")');
...
function add_file_upload(nomor, ongkir)
{
	//alert(ongkir);
...
function save()
{
    ...
    $.ajax({
        url : "<?php echo site_url('kirim_order/add_order_confirmation')?>",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
					...
					<form action="#" id="upload" class="form-horizontal">
						...
						<div class="form-body">
							<div class="form-group">
								<label class="control-label col-md-3">Pilih Jenis Pengiriman</label>
								<div class="col-md-9">
									<?php 
										echo form_dropdown("id_Jenis_Pengiriman", $option_jenis_pengiriman, '' ,'id="id_Jenis_Pengiriman" class="form-control" name="id_Jenis_Pengiriman"'); //name(string), option value(array), selected(array), extra(mixed)  --selectednya dikosongkan
									?>
						...
						<div class="form-body">
							<div class="form-group">
								...
								<div class="col-md-9">
									<input name="Ongkir" id="Ongkir" placeholder="Ongkos Kirim" class="form-control" type="text">
									<span class="help-block"></span>
								</div>
							</div>
						</div>
						<div class="form-body">
							<div class="form-group">
								<label class="control-label col-md-3">No. Resi Pengiriman</label>
								<div class="col-md-9">
									<input name="no_Resi" placeholder="Masukkan Nomor Resi" class="form-control" type="text">
									<span class="help-block"></span>
								</div>
							</div>
						</div>
