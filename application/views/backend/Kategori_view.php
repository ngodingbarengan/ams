...
<!--main content start-->
    <section id="main-content">
        <section class="wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="glyphicon glyphicon-folder-open"></i> DATA KATEGORI</h3>
				</div>
			</div>
            <!-- page start-->
...
function edit_data(id_kategori)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('kategori/ajax_edit')?>/" + id_kategori,
        type: "GET",
        dataType: "JSON",
...
function delete_data(id_kategori)
{
    if(confirm('Apakah kamu yakin akan menghapus data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('kategori/ajax_delete')?>/"+id_kategori,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
