...
<!--main content start-->
    <section id="main-content">
        <section class="wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="glyphicon glyphicon-folder-open"></i> APPROVAL PURCHASE ORDER</h3>
				</div>
			</div>
            <!-- page start-->
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('approval/ajax_list_po')?>",
            "type": "POST"
        },
...
$(document).on('click', '#LihatDetailTransaksi', function(e){
		...
		$.ajax({
			url: "<?php echo site_url('approval/detail_po');?>",
			type: "POST",
			cache: false,
			data: { 
			nomor_po : $(this).text()
			...
			success: function(data){
				...
				jQuery.each(data['master'], function(obj, values) {
				   ...
				   //console.log(data['master'].nomor_po);
					...
					$('.modal-title').text('Rincian PO No : '+data['master'].nomor_po); // Set Title to Bootstrap modal title
					$('.modal-body').empty();
...
function approve_order(nomor_po)
{
	//alert(nomor_po);
    // ajax delete data to database
    $.ajax({
        url : "<?php echo site_url('approval/approve_po')?>/",
        type: "POST",
		data: { 
			nomor : nomor_po
			},
...
function cancel_order(nomor_po)
{
    if(confirm('Apakah kamu yakin akan membatalkan order?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('approval/cancel_po')?>",
            type: "POST",
			data: { 
			nomor : nomor_po
			},
