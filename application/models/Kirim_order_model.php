...
class Kirim_order_model extends CI_Model {
	...
	private function _get_datatables_query() //private
	{
		
		$this->db->select('a.id_order, a.nomor_order, a.tanggal, a.waktu, b.nama_lengkap, a.no_kontak, a.alamat, d.nama_provinsi, e.nama_kota_kab, f.nama_kecamatan, c.username, a.keterangan, a.total_kuantitas, a.total_harga, a.total_diskon, a.total_ppn, a.grand_total, a.ongkir, g.no_resi, h.jenis_pengiriman');
		...
		$this->db->join('tbl_konfirmasi g', 'a.nomor_order = g.nomor_order');
		$this->db->join('tbl_jenis_pengiriman h', 'g.id_jenis_pengiriman = h.id_jenis_pengiriman', 'left outer');
		$this->db->where('status', 'APPROVE');
		$this->db->where_not_in('g.bukti_pembayaran', NULL);
	...
	*/
	...
	public function save_order_confirmation($where, $data)
	{
		$this->db->update('tbl_konfirmasi', $data, $where);
